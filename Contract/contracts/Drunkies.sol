...
contract Drunkies is ERC721URIStorage, Ownable {
    ...
    function checkFreeMintAvailable() public view returns (uint256) {
        uint256 balanceInExternalContract = IERC721(otherNFTContractAddress)
            .balanceOf(msg.sender);
        if (balanceInExternalContract >= freeMintLimit) {
            return balanceInExternalContract;
        } else {
            return 0;
        }
    }
