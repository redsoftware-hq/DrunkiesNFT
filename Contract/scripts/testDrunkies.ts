...
describe('Drunkies', function () {
  ...
  describe('mintNFT', function () {
    ...
    it("should revert if the caller doesn't send enough ether", async function () {
      await expect(drunkies.mintNFT(tokenURI, { value: ethers.utils.parseEther("0.02") })).to.be.revertedWith("Need to send 0.03 ether");
    });
    ...
    it('should withdraw the contract balance to the owner', async function () {
      ...
      const tx = await drunkies.mintNFT(tokenURI, { value: ethers.utils.parseEther('0.03') });
      await tx.wait();
    ...
    it("should mint an nft", async function () {
      ...
      const transferEvent = events.events.find((event: { event: string; }) => event.event === 'Transfer');
    ...
    it("should revert if max supply has been reached", async function () {
      ...
      await drunkies.mintNFT(tokenURI, { value: ethers.utils.parseEther('0.03') });
      await drunkies.mintNFT(tokenURI, { value: ethers.utils.parseEther('0.03') });
      await drunkies.mintNFT(tokenURI, { value: ethers.utils.parseEther('0.03') });

      await expect(drunkies.mintNFT(tokenURI, { value: ethers.utils.parseEther('0.03') })).to.be.revertedWith("Max supply reached");
    });
    ...
    it("should transfer ethers and NFT ownership", async function () {
      ...
      await drunkies.connect(buyer).buyNFT(tokenId, { value: ethers.utils.parseEther('0.03') });
    ...
    it("should revert if the wrong amount of ether is sent", async function () {
      ...
      await expect(drunkies.connect(buyer).buyNFT(tokenId, { value: ethers.utils.parseEther('0.02') }))
        .to.be.revertedWith("Need to send 0.03 ether");
    });
    ...
    it("should revert if the buyer tries to buy their own NFT", async function () {
      ...
      await expect(drunkies.connect(owner).buyNFT(tokenId, { value: ethers.utils.parseEther('0.03') }))
    ...
    it("should revert if the token ID is invalid", async function () {
      ...
      await expect(drunkies.connect(buyer).buyNFT(999, { value: ethers.utils.parseEther('0.03') }))
